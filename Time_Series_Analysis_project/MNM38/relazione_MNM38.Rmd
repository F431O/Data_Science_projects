...
# Analisi serie storica MNM38

### Fabio Palmigiani

Il documento illustra l'analisi della serie storica MNM38, estrapolata dal file "MC1001" che contiene 1001 time series usate per una nota gara di forecast patrocinata dall'IIF.
(<https://forecasters.org/resources/time-series-data/m-competition/>).

Per facilitarne la lettura la serie interessata, è stata estrapolata dal file "MC1001" e inserita in un nuovo file excel chiamato "MNM38".

Le librerie usate sono state omesse da questo documento in modo da renderlo più snello.
(esse sono comunque visibili sul codice MNM38.R)

## Analisi preliminare

La prima cosa che è stata fatta, dopo ovviamente aver imortato il dataset, è "aggiustare" quest'ultimo, sono state infatti aggiunte le colonne mese e anno, una colonna "t" come contatore di tempo ed è stata rinominata la colonna dei valori.
Successivamente è stato diviso il dataset in trainig set e test set, si è deciso di utilizzare l'85% dei dati come training e il restante come test.
...
in modo tale da averne una prima rappresentazione visiva, sono stati disegnati lo scatter plot ed il line plot della serie.
...
Da una prima analisi visiva, sembrerebbe esserci presenza di stagionalità, mentre non salta subito all'occhio la presenza di un forte trend, neppure di eteroschedasticità, ma per approfondire questi temi si è deciso di procedere con un'analisi più approfondita.

### Descrizione del modello tramite funzione lineare

Si è deciso di provare a descrivere il modello tramite una funzione lineare e, per valitarne la bontà ne si studiano i residui.
...
Si genera ora lo scatter plot ed il line plot dei residui della funzione lineare.
...
Last_Date <- paste(Data_df$Month[length],Data_df$Year[length])
title_content <- bquote(atop("Essentials of Time Series Analysis \u0040 Master in Data Science 2022-2023", 
                             paste("Scatter Plot of Residuals of the Linear Model for the Training Set of MNM38")))
subtitle_content <- bquote(paste("path length ", .(length), " sample points."))
caption_content <- "Author: Fabio Palmigiani"
x_name <- bquote("")
y_name <- bquote("Values")
...
Data_df  <- MNM38_train_df
title_content <- bquote(atop("Essentials of Time Series Analysis \u0040 Master in Data Science 2022-2023", 
                             paste("Line Plot of Residuals of the Linear Model for the Training Set of MNM38")))
...
La prima cosa che è saltata all'occhio è la presenza di stagionalità, olte ad una curva LOESS quasi piatta che è una forte evidenza visiva della stazionarietà della media dei residui.
Tuttavia, per verificare computazionalmente la stazionarietà di quest'ultimi si ricorre solitamente a un paio di test: il test Augmented Dickey-Fuller (ADF) e il test Kwiatowski-Phillips-Schmidt-Shin (KPSS).
Il test ADF assume l'ipotesi nulla di non stazionarietà.
Più precisamente, il test ADF assume che la serie temporale sia generata da un processo stocastico con una componente random walk.
Questa ipotesi nulla porta a definire il test ADF come un test di radice unitaria.
Al contrario, il test KPSS assume l'ipotesi nulla di stazionarietà.
In definitiva, il test KPSS ipotizza che la serie temporale sia generata da un processo autoregressivo.
Quindi, quando il test ADF rifiuta l'ipotesi nulla e il KPSS no, abbiamo la prova della stazionarietà della serie temporale.
Quando invece il test ADF non rifiuta l'ipotesi nullo e il KPSS sì, abbiamo evidenza di non stazionarietà.
Gli altri casi sono considerati dubbi.

Si è iniziato con l'eseguire il test di Augmented Dickey-Fuller (ADF):
...
Si è potuto quindi rifiutare l'ipoteri nulla in quanto il valore restituito è più piccolo del valore di riferimento del 1%.

Si è passato ad eseguire il test KPSS:
...
Non si è potuto rifiutare l'ipotesi nulla di stazionarietà, in quanto il valore retituito non è più grande del valore di riferimento del 1%, anzi è addirittura inferiore al valore di riferimento del 10%.

In definitiva si è potuto affermare che i residui del modello lineare MNM38_lm sono stati generati da un processo con media stazionaria (zero).

Si è dusegnato ora il grafico a dispersione delle radici quadrate dei residui:

```{r, echo=TRUE}
plot(MNM38_lm,3)
```

Si è osservato che i punti sono equamente distribuiti attorno a una linea orizzontale, senza evidenza di schemi distinti.
Questo è una prima evidenza visiva dell'omoshedasticità nei residui. Inoltre, si è notato una curva LOESS rossa molto vicina alla linea orizzontale, questa è un'altra prova visiva dell'assenza di eteroshedasticità.

Si è passato a disegnare uno scatter plot ed un line plot delle radici quadrate dei residui rispetto ai valori fittati:
...
Last_Date <- paste(Data_df$Month[length],Data_df$Year[length])
title_content <- bquote(atop("Essentials of Time Series Analysis \u0040 Master in Data Science 2022-2023", 
                             paste("Scatter plot delle radici quadrate dei residui rispetto ai valori fittati MNM38.")))
subtitle_content <- bquote(paste("path length ", .(length), " sample points."))
caption_content <- "Author: Fabio Palmigiani"
...
Last_Date <- paste(Data_df$Month[length],Data_df$Year[length])
title_content <- bquote(atop("Essentials of Time Series Analysis \u0040 Master in Data Science 2022-2023",
                             paste("Line plot delle radici quadrate dei residui rispetto ai valori fittati MNM38.")))
x_name <- bquote("")
x_breaks_num <- 30
x_breaks_low <- Data_df$t[1]
x_breaks_up <- Data_df$t[length]
...
Si è passato a studiare in maniera computazionale l'omoshedasticità dei residui, effettuando il test di reusch-Pagan e il W test:
...
entrambi i test hanno un p-value alto, quindi non possimao rifiutare l'ipotesti di omoschedasticità.

Per studiare l'autocerrelazione dei residui si è disegnato l'autocorrelogramma:
...
Last_Date <- paste(Data_df$Month[length],Data_df$Year[length])
title_content <- bquote(atop("Essentials of Time Series Analysis \u0040 Master in Data Science 2022-2023", 
                             paste("Autocorrelogramma dei residui del modello lineare di MNM38")))
x_name <- bquote("Lags")
x_breaks_num <- maxlag
x_binwidth <- 1
...
Dall'autocorrelogramma si evince una forte presenza di stagionalità, tanto da rendere inutile studiare l'autocorrelogramma parziale.
Per confermare questa evidenza si è effettuato il test di Ljung-Box (LB):
...
Il test conferma una forte autocorrelazione dei residui.

A questo punto, si è tornato alla serie originale.

Siccome l'unica cosa che bisogna cercare di eliminare è la forte presenza di stagionalità, visto che i residui risultavano essere omoschedastici e stazionari, non c'è stata la necessità di fare la trasformazione in Box-Cox, ma si è proceduto con la decomposizione della serie.
...
In questa fase si sono tolte le componenti di trend e stagionalità dalla serie.
...
Grafico della componente media annua del terend e della componente trend della serie storica:
...
Si è rimosso quindi il trend dalla serie:
...
Ora si sono effettuati gli stessi passaggi per eliminare la stagionalità dalla serie:
...
Componente di stagionalità della serie:

```{r, echo=TRUE}
plot(MNM38_seas_ts, col="blue", xlab="Date", ylab="Value", main="MNM38 - Componente stagionalità")
```

Si è proceduto con la costruzione del dataset destagionalizzato:

```{r, recho=TRUE}
MNM38_deseas_ts <- MNM38_ts-MNM38_seas_ts
```

Gragico della serie destagionalizzata:

```{r, echo=TRUE}
plot(MNM38_deseas_ts, xlab="Time", ylab="", main="MNM38 - Serie destagionalizzata")
```

Infine si è costruito la serie detrendizzata e destagionalizzata, ovvero la componente residua della time series:

```{r echo=TRUE}
MNM38_res_ts <- MNM38_ts - MNM38_trend_ts - MNM38_seas_ts
plot(MNM38_res_ts, xlab="Time", ylab="", main="MNM38 - Serie destagionalizzata e detrendizzata")
...
### Studio dei residui della serie destagionalizzata e detrendizzata

Line plot del rumore della serie:
...
Si è eseguito il test di stazionarietà:
...
Come ci si aspettava, i residui sono stati generati da rumori stazionari.

Si è proceduto con i test di omoschedasticità:
...
quindi non è stato possibile rifiutare l'ipotesti di omoschedasticità.

In modo tale da avere evidenza visiva del fatto che essi siano autocorrelati o meno, si è disegnato l'autocorrelogramma e l'autocorrelogramma parziale dei residui:
...
A prima vista sembrerebbe esserci presenza di autocorrelazione, ma per esserne sicuri si è effettuato il test di Ljung-Box:
...
Il test conferma una lieve presenza di autocorrelazione tra i residui.

Si è decide quindi di usare il modello ARIMA per descrivere la serie.

### Modello ARIMA

Si è autogenerato il miglior modello possibile per la serie:
...
Il miglior modello è risultato essere ARIMA(3,0,1).

Si è passato a studiare l'autocorrelazione dei residui del modello ARIMA(3,0,1), disegnando l'autocorrelogramma e l'autocorrelogramma parziale dei residui del modello ARIMA(3,0,1):
...
Last_Date <- paste(Data_df$Month[length],Data_df$Year[length])
title_content <- bquote(atop("Essentials of Time Series Analysis \u0040 Master in Data Science 2022-2023", 
                             paste("Autocorrelogramma dei residui del modello ARIMA(3,0,1) di MNM38" )))
subtitle_content <- bquote(paste("path length ", .(length), " sample points,   ", "lags ", .(maxlag)))
ggplot(Plot_Aut_Fun_y, aes(x=lag, y=acf)) + 
  geom_segment(aes(x=lag, y=rep(0,length(lag)), xend=lag, yend=acf), size = 1, col="black") +
  geom_hline(aes(yintercept=-ci_90, color="CI_90"), show.legend = TRUE, lty=3) +
...
Last_Date <- paste(Data_df$Month[length],Data_df$Year[length])
title_content <- bquote(atop("Essentials of Time Series Analysis \u0040 Master in Data Science 2022-2023", 
                             paste("Autocorrelogramma parziale dei residui del modello ARIMA(3,0,1) di MNM38" )))
subtitle_content <- bquote(paste("path length ", .(length), " sample points,   ", "lags ", .(maxlag)))
ggplot(Plot_P_Aut_Fun_y, aes(x=lag, y=pacf)) + 
  geom_segment(aes(x=lag, y=rep(0,length(lag)), xend=lag, yend=pacf), size = 1, col="black") +
  geom_hline(aes(yintercept=-ci_90, color="CI_90"), show.legend = TRUE, lty=3) +
...
Dai due autocorrelogrammi c'è evidenza di non autocorrelazione dei residui del modello ARIMA(3,0,1), cosa confermata anche dal test di Ljung-Box.

Si è continuato a studiare la stazionarietà dei residui:
```{r, echo=TRUE}
# ADF Test
...
In definitiva quindi non è stato possibile rigettare l'ipotesi che afferma che i residui del odello ARIMA(3,0,1) siano stati generati da rumori stazionari.

A questo punto bisognava capire quale distribuzione spiega meglio i residui del modello ARIMA(3,0,1).
E' stato assunto che essi siano descritti da una distribuzione Gaussiana e, per confermare questa ipotesi, sono stati eseguiti i test di Shapiro-Wilk, D’Agostino-Pearson, Anderson-Darling e Jarque Bera:
```{r, echo=TRUE}
...
Tutti e tre i test suggeriscono di rifiutare l’ipotesi nulla di Gaussianità. 

A questo punto si è diseganto l'istogramma dei residui standardizzati del modello ARIMA(3,0,1) per avere una prima evidenza visiva:
```{r, echo=TRUE}
...
Si è mostrato anche la funzione di ripartizione empirica dei residui standardizzati del modello ARIMA(3,0,1):
```{r, echo=TRUE}
...
Dai grafici ottenuti si è indotti a pensare che la distribuzione migliore sia quella di student.

Per confermare questa tesi sono stati stimati i parametri della distribuzione teorica e si è confrontata la sua densità con quella empirica.
```{r echo=TRUE}
...
Dal gragico le due densità sembrano essere molto simili, quindi sembrerebbe corretta l'ipotesi he i residui del modello ARIMA(3,0,1) siano descritti dalla distribuzione di student, per avvalorare ulteriormente l'ipotesi si sono effettuati i test di goodness of fit:
```{r, echo=TRUE}
KS_x_st_t_ls <- ks.test(z_st, y="pt_ls", m=m, s=s, df=df, alternative="two.sided")
show(KS_x_st_t_ls)
```
```{r, echo=TRUE}
...
Tutti e tre i test hanno p-value > 0.05, addirittura sono molto vicini all'1, dunque non è stato possibile rifiutare l’ipotesi nulla che la distribuzione sia una distribuzione di student. 

Infine, come ulteriore conferma, si è disegnato il plot nel quale si vede come i quantili empirici sono concordi con i quantili teorici della distribuzione di Student.
```{r, echo=TRUE}
x <- z_st_qemp
...
# aggiunta della linea interquantile
probs <- c(0.25,0.75)
...
Si è processo con la predizione del trend e della stagionalità:
```{r, echo=TRUE}
# PREDIZIONE TREND
t <- 1:TS_length
...
Si è effettuata una previsione dei residui del modello STL utilizzando il modello ARIMA(3,0,1). 
Per prima cosa sono stati recuperati i dati del training set, che serviranno per effettuare le previsione dei residui e successivamente per creare il grafico con la previsione.

Per definire i quantili e la previsione puntuale, che verranno utilizzati per disegnare le bande di previsione, sono state generate 10000 previsioni.
```{r, echo=TRUE}
...
La previsione puntuale è stata definita come la media delle 10000 previsioni puntuali. 
Inoltre, sommando tra loro le previsioni di trend, stagionalità e residui, si è ottenuto la previsione puntuale e le bande di previsione del 90% e del 95% della serie.
```{r echo=TRUE, message=FALSE, warning=FALSE}
...
Si è concluso quindi lo studio della serie, disegnando il grafico finale che mostra la previsione puntuale e le bande di confidenza:
```{r, echo=TRUE}
...
# Conclusione
Guardando al grafico della predizione si nota che il test set rientra all'interno delle bande di predizione del 90%, inoltre è interessante notare che il test set e la predizione si comportano in maniera più o meno simile.
Questo fa pensare al fatto che il modello descriva in manira buona la serie.
Inoltre il test set rientra per la maggior parte all'interno delle bande di predizione del 90%